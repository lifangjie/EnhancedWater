...
namespace WaterVersionFirst {
    ...
    public class Water : MonoBehaviour {
        ...
        private RenderTexture _mDepthTexture;
        ...
        private int _mOldDepthTextureSize;
        private static bool _sInsideWater;


        private Shader _renderDepth;

        private void Start() {
            _renderDepth = Shader.Find("Hidden/RenderDepth");
        }
        ...
        public void OnWillRenderObject() {
            ...
            if (mode >= WaterMode.Refractive) {
                ...
                if (_renderDepth != null) {
                    refractionCamera.targetTexture = _mDepthTexture;
                    refractionCamera.RenderWithShader(_renderDepth, "RenderType");
                    GetComponent<Renderer>().sharedMaterial.SetTexture("_DepthTex", _mDepthTexture);
                }
            }
        ...
        void OnDisable() {
            ...
            if (_mDepthTexture) {
                DestroyImmediate(_mDepthTexture);
                _mDepthTexture = null;
            }
        ...
        void CreateWaterObjects(Camera currentCamera, out Camera reflectionCamera, out Camera refractionCamera) {
            ...
            if (mode >= WaterMode.Refractive) {
                ...
                if (!_mDepthTexture || _mOldDepthTextureSize != TextureSize) {
                    if (_mDepthTexture) {
                        DestroyImmediate(_mDepthTexture);
                    }
                    _mDepthTexture =
                        new RenderTexture(TextureSize, TextureSize, 16, RenderTextureFormat.ARGBFloat) {
                            //new RenderTexture(TextureSize, TextureSize, 16) {
                            name = "__WaterDepth" + GetInstanceID(),
                            isPowerOfTwo = true,
                            hideFlags = HideFlags.DontSave
                        };
                    _mOldDepthTextureSize = TextureSize;
                }
